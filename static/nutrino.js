...
window.pluginRegistry.push({
  ...
  init: function () {
    if (typeof socket === "undefined") {
      console.warn("[nutrino] socket.io client not found");
      return;
    }
    ...
    if (devices.size === 0) {
      console.warn("[nutrino] No elements with data-device");
      return;
    }
    ...
    devices.forEach(deviceName => {
      ...
      if (!statusEl || !msgEl) {
        console.warn(`[nutrino] Missing containers for ${deviceName}`);
        return;
      }
